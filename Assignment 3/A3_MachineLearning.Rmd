...
true_val <- function(means, SDs){
  ...
  names(true_df) <- c(paste0("v", seq(10),"_"))
...
#Redrawing individual effect sizes with trial variability (and measurement error)
sim_d <- sim_d %>% rowwise %>% mutate(across(starts_with("v"), ~rnorm(1, mean=rnorm(1,mean=.x, sd=trial_error), sd= m_error), .names = "{.col}_obs"))

sim_d_1 <- sim_d %>% rowwise %>% mutate(across(starts_with("v"), ~rnorm(1, mean=.x, sd= trial_error), .names = "{.col}_obs"))
