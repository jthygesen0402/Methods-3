...
#train_noise
train_noise$av_pred_perc <- predict(m1_noise)[,1] #new column with average predictions
...
train_n <- inv_logit_scaled(posterior_linpred(m1, newdata = train_noise, summary = F, allow_new_levels = T))
test_n <- inv_logit_scaled(posterior_linpred(m1, newdata = test_noise_s, summary = F, allow_new_levels = T))
```


```{r}
fun_df <- function(x){
  if(x[,"data"] == "informed" & x[,"type"] == "training"){var <- accuracy(train_inf, truth = group, estimate = av_pred)[,"estimate"]}
    
  if(x[,"data"] == "informed" & x[,"type"] == "test"){ var <- accuracy(test_inf_s, truth = group, estimate = av_pred)[,"estimate"]}

  if(x[,"data"] == "noise" & x[,"type"] == "training"){ var <- accuracy(train_noise, truth = group, estimate = av_pred)[,"estimate"]}
  
  if(x[,"data"] == "noise" & x[,"type"] == "test"){ var <-  accuracy(test_noise_s, truth = group, estimate = av_pred)[,"estimate"]}
  
  return(var)
}


#map(perf_prob, fun_df)

train_inf$predictions
...
for (i in seq(4000)){
  ...
  perf_prob$accuracy[perf_prob$sample==i & perf_prob$data == "informed" & perf_prob$type=="training"] <- accuracy(train_inf, truth = group, estimate = predictions)[, ".estimate"]
   ...
   perf_prob$accuracy[perf_prob$sample==i & perf_prob$data == "informed" & perf_prob$type=="test"] <- accuracy(test_inf_s, truth = group, estimate = predictions)[, ".estimate"]
    ...
    perf_prob$accuracy[perf_prob$sample==i & perf_prob$data == "noise" & perf_prob$type=="training"] <- accuracy(train_noise, truth = group, estimate = predictions)[, ".estimate"]
     ...
     perf_prob$accuracy[perf_prob$sample==i & perf_prob$data == "noise" & perf_prob$type=="test"] <- accuracy(test_noise_s, truth = group, estimate = predictions)[, ".estimate"]
}
```
...
These are just the average predictions. And our model shows us that it can find predictions on noise, which is problematic. . But this is without including uncertainty. 

We need to generate predictions of every posterior samples of our model and plot the full distribution of the performance/accuracy. 
