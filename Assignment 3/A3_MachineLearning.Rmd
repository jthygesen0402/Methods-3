...
```{r}
relevant_cols <- t %>% dplyr::select(Harmonicity_SD, MCEP2_Mean,Clarity_Mean, HMPDD0_SD, MCEP11_Median,  MCEP8_Mean, CreakProbability_SD, F0_SD_Praat, Pitch_IQR, TurnNumMin_Cova)

train_select <- cbind(train_factor, relevant_cols)
```

```{r}
f1_real <- bf(Diagnosis ~ 1 + Harmonicity_SD+ MCEP2_Mean+Clarity_Mean+ HMPDD0_SD+ MCEP11_Median+  MCEP8_Mean+ CreakProbability_SD+ F0_SD_Praat+ Pitch_IQR+ TurnNumMin_Cova + (1+Trial|NewID))
```
