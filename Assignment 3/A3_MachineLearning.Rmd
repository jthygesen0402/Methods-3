...
# Assessing performance on the  data
```{r}

#train_inf
train_inf$av_pred_perc <- predict(m1)[,1] #new column with average predictions
train_inf <- train_inf %>% mutate(pred =as.factor(ifelse(av_pred_perc > 0.5, "HC", "SZ")))
...
train_noise <- train_noise %>% mutate(pred =as.factor(ifelse(av_pred_perc > 0.5, "HC", "SZ")))
...
m1_noise_test <- update(m1, newdata = test_noise )
...
```{r}
#inf
test_inf$av_pred_perc <- predict(m1_inf_test, allow_new_levels=T)[,1]
test_inf <- test_inf %>% mutate(pred =as.factor(ifelse(av_pred_perc > 0.5, "HC", "SZ")))

test_inf$pred <- factor(test_inf$pred, levels=c('SZ', 'HC'))



#noise
test_noise$av_pred_perc <- predict(m1_noise_test, allow_new_levels=T)[,1]
test_noise <- test_noise %>% mutate(pred =as.factor(ifelse(av_pred_perc > 0.5, "HC", "SZ")))

test_noise$pred <- factor(test_noise$pred, levels=c('SZ', 'HC'))
```
...
```{r}
#Creating the engine
pacman::p_load(randomForest, kernlab)


train_trees <- train_inf %>% mutate(ID = NULL, trial = NULL, av_pred = NULL, v3_obs_matched = NULL)
...
am_i_important <- explain_tidymodels(informed_trees, 
                                     data = train_inf, 
