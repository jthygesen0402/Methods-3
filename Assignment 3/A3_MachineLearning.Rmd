...
```{r}
#Creating a funtion that can simulate the true underlying value for all variables for a participant
...
# For each trial, simulate a value that is drawn from the undelying mean with TrialSD as SD and simulate error on top of that
...
```{r}
#Differentiating between SZ and HC 
matched_data_noise<- sim_d_noise %>% 
  rowwise %>%  
  mutate(across(ends_with("_obs"), ~if_else(group == "SZ", (.x/2), -(.x/2)), .names ="{.col}_matched")) %>%
  select(ID, trial, group, ends_with("_matched"))
...
### Noise data
```{r}
#Making the noise df into long format 
long_df_noise <- matched_data_noise %>%
  pivot_longer(cols = ends_with("_matched"), names_to = "variable")
...
test_inf <- bake(recipe_inf, new_data = test_inf)
```
...
```{r}
#Splitting data 80/20
train_noise <- matched_data_noise %>% subset(!(ID %in% spl)) %>% mutate(ID=as.factor(ID), trial=as.factor(trial))
test_noise <- matched_data_noise %>% subset(ID %in% spl) %>% mutate(ID=as.factor(ID), trial=as.factor(trial))
...
test_noise <- bake(recipe_noise, new_data = test_noise)
...
u_train <- tibble(train_inf = predict(m1)[,2], train_noise = predict(m1_noise)[,2]  )  

u_test <- tibble(test_inf = predict(m1, newdata = test_inf, allow_new_levels=T)[,2], test_noise = predict(m1_noise, newdata = test_noise, allow_new_levels=T)[,2])
