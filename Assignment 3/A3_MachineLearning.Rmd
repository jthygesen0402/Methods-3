...
```{r}
pacman::p_load(tidyverse,purrr, brms, cmdstanr, bayesplot, rstanarm, viridis, gridExtra, RColorBrewer, reshape2, tidymodels, yardstick, corrplot, DALEX, DALEXtra, randomForest, xgboost, kernlab, caret, cvms, groupdata2, DataExplorer, klaR)
...
stratify
cvms - balancing data, nice on continuous variables
...
y_train <- train_real$Diagnosis

x_train <-train_real[,-c(395,396)] %>% as.data.frame()
...
pca_train_ <-  recipe(Diagnosis ~., data = train_part) %>% 
...
low <- pca_train_2 %>% filter(value < 0.01 | value > -0.01)
high <- pca_train_2 %>% filter(value >0.01 | value < -0.01)
