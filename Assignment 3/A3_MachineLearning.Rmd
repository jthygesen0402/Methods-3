...
```{r}
pacman::p_load(tidyverse,purrr, brms, cmdstanr, bayesplot, rstanarm, viridis, gridExtra, RColorBrewer, reshape2, tidymodels, yardstick, corrplot, DALEX, DALEXtra)
...
train_inf <- matched_data %>% subset(!(ID %in% spl)) %>% mutate(ID = as.factor(ID), trial = as.factor(trial))
test_inf <- matched_data %>% subset(ID %in% spl) %>% mutate(ID = as.factor(ID), trial = as.factor(trial))
...
train_inf <- juice(rec_informed)
test_inf <- bake(rec_informed, new_data = test_inf)
```
...
```{r}
#Splitting data 80/20
train_noise <- matched_noise %>% subset(!(ID %in% spl)) %>% mutate(ID = as.factor(ID), trial = as.factor(trial))
test_noise <- matched_noise %>% subset(ID %in% spl) %>% mutate(ID = as.factor(ID), trial = as.factor(trial))
...
#Using the recipe on the test data
train_noise <- juice(rec_noise)
test_noise <- bake(rec_noise, new_data = test_noise)
...
#look at v_8 (noise variable) - do we revisit the priors when it's noise??
```
...
```{r}
m1_noise_train <- update(m1, newdata = train_noise)
pp_check(m1_noise_train, ndraws=1000)
print(m1_noise_train)

```

```{r}
m1_posterior_noise <- as_draws_df(m1_noise_train) 
...
grid.arrange(plot7_noise, plot8_noise, plot9_noise, plot10_noise)
```


*NB:*Do we redo our formula and/or our priors before checking the performance of it ????
...
#train_inf
train_inf$av_pred_perc <- predict(m1)[,1] #new column with average predictions
...
#train_noise
train_noise$av_pred_perc <- predict(m1_noise_train)[,1] #new column with average predictions
train_noise <- train_noise %>% mutate(av_pred =as.factor(ifelse(av_pred_perc > 0.5, "HC", "SZ")))

train_noise$av_pred <- factor(train_noise$av_pred, levels=c('SZ', 'HC'))
...
u_train <- tibble(train_inf = predict(m1)[,2], train_noise = predict(m1_noise_train)[,2]  )  

u_test <- tibble(test_inf = predict(m1, newdata = test_inf, allow_new_levels=T)[,2], test_noise = predict(m1_noise_train, newdata = test_noise, allow_new_levels=T)[,2])
...
# Now running the model on the test data - both informed and noise
```{r}
#m1_inf_test <- update(m1, newdata = test_inf)
#m1_noise_test <- update(m1, newdata = test_inf )
```
...
test_inf$av_pred_perc <- predict(m1, newdata = test_inf, allow_new_levels=T)[,1]
test_inf <- test_inf %>% mutate(av_pred =as.factor(ifelse(av_pred_perc > 0.5, "HC", "SZ")))

test_inf$av_pred <- factor(test_inf$av_pred, levels=c('SZ', 'HC'))



#noise

test_noise$av_pred_perc <- predict(m1, newdata = test_noise, allow_new_levels=T)[,1]
test_noise <- test_noise %>% mutate(av_pred =as.factor(ifelse(av_pred_perc > 0.5, "HC", "SZ")))

test_noise$av_pred <- factor(test_noise$av_pred, levels=c('SZ', 'HC'))
```
...
yardstick::conf_mat(test_noise, 
         truth = group,
         estimate = av_pred,
         dnn = c("Prediction", "Truth"))
```

#Check correlation between predictors - the frequentest way - w
```{r}
cor_dat <- matched_data %>%  select(starts_with("v"))

corrrr <- correlations(cor_dat, use ="everything" , method = "pearson")


corrplot(corrrr, method = "number")
```
...
```{r}
#Creating the engine
pacman::p_load(randomForest, xgboost, kernlab)


train_trees <- train_inf %>% mutate(ID = NULL, trial = NULL, av_pred = NULL, av_pred_perc = NULL, v3_obs_matched = NULL)
...
am_i_important_imp <- model_parts(am_i_important) 

plot(am_i_important_imp)
```
...
LogisticRegression <- logistic_reg() %>%
  set_mode("classification") %>% 
  set_engine("glm") %>%
  fit(Diagnosis ~ . , data = real_sub_data)

dd <- explain_tidymodels(LogisticRegression, 
                                     data = real_sub_data, 
                                     y= as.numeric(real_sub_data$group)-1,
                                     label= "GLM", 
                                     verbose = FALSE)
...
caret::findLinearCombos(real_sub_data)
```
