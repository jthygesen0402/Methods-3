...
```{r}
pacman::p_load(tidyverse,purrr, brms, cmdstanr, bayesplot, rstanarm, viridis, gridExtra, RColorBrewer, reshape2, tidymodels, yardstick, corrplot, DALEX, DALEXtra, randomForest, xgboost, kernlab, caret, cvms, groupdata2)
...
```{r}
train_inf_pred <- train
test_inf_s_pred <- test
train_noise_pred <- train_n
test_noise_s_pred <- test_n

for (i in seq(1000)){
  train_inf_pred[i,] <-  ifelse(train[i,] > 0.5, "HC", "SZ")
  test_inf_s_pred[i,] <- as.factor(ifelse(test[i,] > 0.5, "HC", "SZ")) %>% factor(levels=c('SZ', 'HC'))
  train_noise_pred[i,] <- as.factor(ifelse(train_n[i,] > 0.5, "HC", "SZ")) %>%  factor(levels=c('SZ', 'HC'))
  test_noise_s_pred[i,] <- as.factor(ifelse(test_n[i,] > 0.5, "HC", "SZ")) %>% factor(levels=c('SZ', 'HC'))
}  

train_inf_pred[sapply(train_inf_pred, is.character)] <- lapply(train_inf_pred[sapply(train_inf_pred, is.character)], as.factor)

  train_inf$accuracy <- train_inf %>% rowwise() %>% accuracy(truth=group,estimate=predictions)
  test_inf_s$accuracy <- test_inf_s %>% rowwise()%>% accuracy(truth=group,estimate=predictions)
  train_noise$accuracy <- train_noise%>% rowwise() %>% accuracy(truth=group,estimate=predictions)
  test_noise_s$accuracy <- test_noise_s %>% rowwise()%>% accuracy(truth=group,estimate=predictions)
...
accuracy(test_noise_s, truth = group, estimate = predictions)[, ".estimate"]


t <- as.factor(ifelse(test_n[1,] > 0.5, "HC", "SZ")) %>% factor(levels=c('SZ', 'HC'))
test_inf_s$predictions
```
...
ggplot(perf_prob, aes(y=as.numeric(accuracy) , x=type)) +
...
^^ Global importance - generally, which are the features that matter the most?
...
```
stratify
cvms - balancing data, nice on continous variables
