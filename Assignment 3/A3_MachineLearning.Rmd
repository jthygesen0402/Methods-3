...
```{r}
pacman::p_load(tidyverse,purrr, brms, cmdstanr, bayesplot, rstanarm, viridis, gridExtra, RColorBrewer, reshape2, tidymodels, yardstick, corrplot, DALEX, DALEXtra, randomForest, xgboost, kernlab)
...
>>>>>>> 51181944a97d05cef1f69b0986de76aa5b07955d
...
<<<<<<< HEAD
sim_d_noise <- sim_d_noise %>% rowwise %>% mutate(across(starts_with("v"), ~rnorm(1, mean = rnorm(1,mean=.x, sd=TrialSD), sd = Error), .names="{.col}_obs"))
...
f <- function(x){
  train_inf$av_pred <- as.factor(ifelse(train >0.5, "HC", "SZ"))
  test_inf_s$av_pred <- as.factor(ifelse(train >0.5, "HC", "SZ"))
  
  perf_prob$accuracy <- if(perf_prob$sample==x & perf_prob$data == "informed" & perf_prob$type == "training", accuracy(train_inf, truth = group, estimate = av_pred))
}
