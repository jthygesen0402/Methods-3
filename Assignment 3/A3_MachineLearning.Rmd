...
sim_d <- sim_d %>% rowwise %>% mutate(across(starts_with("v"), ~rnorm(1, mean = .x, sd = TrialSD), .names="{.col}_obs"))
```
...
sim_d_noise <- sim_d_noise %>% rowwise %>% mutate(across(starts_with("v"), ~rnorm(1, mean = .x, sd = TrialSD), .names="{.col}_obs"))
```
