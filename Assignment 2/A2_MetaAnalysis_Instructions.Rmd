...
Describing the data
```{r}
#stacked barplot of number of participants
pacman::p_load(viridis, hrbrthemes)


as.factor(real_data$StudyID)
as.integer(real_data$SAMPLE_SIZE_HC, real_data$SAMPLE_SIZE_SZ, real_data$SAMPLE_SIZE_TOTAL)

ggplot(data=real_data, aes(x=StudyID, y=SAMPLE_SIZE_TOTAL)) +  geom_bar(stat="identity")  +
  geom_text(aes(label=SAMPLE_SIZE_TOTAL), vjust=1.6, color="white",
