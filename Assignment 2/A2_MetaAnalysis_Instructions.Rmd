...
```{r}
pacman::p_load(tidyverse,msm, brms, gridExtra, bayesplot, rstanarm, readxl, tidyr)
...
Population effects
```{r}
real_data_pitch <- mutate(real_data_pitch, effect_size=NA)


for (i in seq(length(real_data_pitch))) {
  real_data_pitch$effect_size[i] <- (real_data_pitch$PITCH_F0SD_HC_M[i]-real_data_pitch$PITCH_F0SD_SZ_M[i])/sqrt((real_data_pitch$PITCH_F0SD_HC_SD[i]^2+real_data_pitch$PITCH_F0SD_SZ_M[i]^2)/2)
}
