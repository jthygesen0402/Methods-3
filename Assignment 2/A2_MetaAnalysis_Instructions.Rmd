...
for (i in seq(studies_n)) {
  ...
  d$effect_mu[i] <- mean(sampling)
  d$effect_sigma[i] <- sd(sampling)/sqrt(d$participants[i])
...
geom_histogram(aes(prior_b_Intercept), fill="steelblue", color="black",alpha=0.6, bins = 30) +
geom_histogram(aes(b_Intercept), fill="#FC4E07", color="black",alpha=0.6, bins = 30) +
xlab('Intercept') +
labs(title="Prior-posterior update check of the intercept") +
theme_classic()


p2 <- ggplot(m1_posterior) +
geom_histogram(aes(prior_sd_study__Intercept), fill="steelblue", color="black",alpha=0.6, bins = 30) +
