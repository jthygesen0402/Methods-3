...
=======
author: "Geri, Natasha, Emilie, Marton, Jakob"
>>>>>>> df49984552946e848566ff63ee9460a29558ccb2
...
clean_visit function(data, num){
  filter(data, VISIT)
}

substring(LU_train$VISIT, 6,6)
```
...
merged_sub <- merged %>% 
  ...
  rename(ADOS1 = ADOS,
         ...
         SOCIALZATION1 = SOCIALIZATION)

merged_sub <- merged_sub %>% select(!VISIT)

merged__ <- merge(merged, merged_sub)
```
...
merged__$ID <- digest(merged__$ID, algo="md5", serialize = F)
```
