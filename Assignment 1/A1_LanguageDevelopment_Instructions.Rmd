...
```{r}
mu_ASD <- log(1.5)
sd_ASD <- log(0.5)-log(1.5-0.5)

mu_TD <- log(1.5)
sd_TD <- log(0.3)-log(1.5-0.3)

mu_change_ASD <- 0.2 #before 0.4 - beta
sigma_change_ASD <- 0.05 #before 0.4 - sd_beta

mu_change_TD <- 0.6 #before 0.6
sigma_change_TD <- 0.3 #before 0.2
...
simulation <- function(n){
  d <- tibble(diagnosis = rep(c("ASD", "TD"), each = n/2), ID= seq(n)) %>%
    mutate(intercept=ifelse(diagnosis=="ASD", rnorm(n, mean=mu_ASD, sd=sd_ASD), rnorm(n, mean=mu_TD, sd=sd_TD)), slope=ifelse(diagnosis=="ASD", rnorm(n, mean=mu_change_ASD, sd=sigma_change_ASD), rnorm(n, mean=mu_change_ASD, sd=sigma_change_TD))) %>% 
    sapply(rep,times=6) %>% 
    as_tibble() %>% 
  ...
  for(i in seq(nrow(d))){
    d$MLU[i] <- exp(rnorm(1, d$intercept[i]+d$slope[i]*(d$visit[i]-1), error))
  }
