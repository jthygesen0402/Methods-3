...
pacman::p_load(tidyverse, brms, bayesplot, rstanarm, zoom, pak, gridExtra, purrr, dplyr, cmdstanr)
install_cmdstan()
...
>>>>>>> 24b8d080aecfb4c82d5528aea6de2950196e6541
```
...
Sample size 70
```{r}
sample70 <- tibble(seed = 1:n_sim) %>%
  mutate(d = map(seed, simulation, n = 70))%>% 
  mutate(fit = map2(d, seed, ~update(MLU_2, newdata = .x, seed = .y)))

```

```{r}
parameters70 <- sample70 %>% mutate(estimate = map(MLU_2, ~ fixef(.) %>% 
                           data.frame() %>%
                           rownames_to_column("parameter"))) %>% unnest(estimate)
```
