...
pp_check(MLU_prior_model, ndraws =100) + xlim(c(-1,75)) 
```
...
df_real_data <- df_real_data %>%  dplyr::rename(diagnosis = Diagnosis, 
                                         visit = Visit,
                                         MLU = CHI_MLU,
                                         ID = Child.ID)
```
...
```{r}
#Adding verbalIQ and nonverbalIQ as predictors
