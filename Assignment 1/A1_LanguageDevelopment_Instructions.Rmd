...
pacman::p_load(tidyverse, brms, bayesplot, rstanarm, zoom,pak, gridExtra)
...
simulation <- function(seed, n){
  set.seed(seed)
  d <- tibble(diagnosis = rep(c("ASD", "TD"), each = n/2), ID= seq(n)) %>%
...
samples <- c(30,40,50,60,70,80,90,100)
```

```{r}
sample60 <- tibble(seed = 1:n_sim) %>%
  mutate(d = map(seed, simulation, n = 60)) %>% 
  mutate(fit = map2(d, seed, ~update(MLU_2, newdata = .x, seed = .y, iter=1000)))
...
```{r}
sample60 <- tibble(seed = 1:n_sim) %>%
  mutate(d = purrr::map(seed, simulation, n = 60))%>% 
  mutate(fit = purrr::map2(d, seed, ~update(MLU_2, newdata = .x, seed = .y, iter=10)))

parameters <- sample60 %>% 
  mutate(estimate = map(fit, ~ fixef(.) %>%
                          data.frame() %>%
...
```{r}
#Plotting the 10 iterations of a sample size of 60 children
parameters %>% 
  filter(parameter == "diagnosis") %>% 
...
>>>>>>> 0a5dc8a32f0ca748fad8be296aedf5841b225b54
```{r}
#Power analysis
