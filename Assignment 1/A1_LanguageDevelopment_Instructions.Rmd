...
data_sim <- function(n, mean, beta){
  df <- tibble(Diagnosis = rep(c("ASD", "TD"), each = n)) %>% 
    mutate(Count = ifelse(Diagnosis == "ASD", rlnorm(n, meanlog = mean, sdlog = sd), rlnorm(n, meanlog = mean + beta, sdlog = sd))) %>% 
...
#Jakob's version
data_sim <- function(n, mean, sd, beta, sdbeta){
  df <- tibble(Diagnosis = rep(c("ASD", "TD"), each = n)) %>% 
    mutate(MLU = ifelse(Diagnosis == "ASD", rlnorm(n, meanlog = mean, sdlog = sd), rlnorm(n, meanlog = mean + rnorm(1,mean=beta, sd=sdbeta), sdlog=sd))) %>% 
    mutate(beta = beta , mean = mean, sd = sd, n = n)
  
  return(df)
}


data_sim(n = 10, mean = 1.5,sd=0.5, beta = 0.4, sdbeta = 0.4)
