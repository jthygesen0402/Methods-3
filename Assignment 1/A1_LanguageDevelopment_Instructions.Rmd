...
```{r}
#Defining the priors - we expect the ASD children to vary more than the TD-children
MLU_priors <- c(
  brms::prior(normal(0,0.2), class = b),
  brms::prior(normal(0.4,0.10), class = b, coef = "diagnosisASD"),
  brms::prior(normal(0.4,0.05), class = b, coef = "diagnosisTD"),
...
MLU_2 <- brm(
  f1,
  data =df,
  family = lognormal, #our prior predictive gets wickedly high values
    sample_prior = T,
   chains = 2,
  cores = 2,
  control = list(
...
sample60 <- tibble(seed = 1:n_sim) %>%
  ...
  mutate(fit = map2(d, seed, ~update(MLU_2, newdata = .x, seed = .y, iter=10)))

parameters <- sample60 %>% mutate(estimate = map(fit, ~ as_draws_df(.)))


parameters$`b_diagnosisASD:visit`-parameters$estimate$`b_diagnosisTD:visit`)

parameters$difference
  
parameters$seed <- as.factor(parameters$seed)
...
parameters_diff <- data.frame(seed=NA,diff_Estimate=NA,diff_Est.Error=NA,diff_Q2.5=NA,diff_Q97.5=NA)
...
parameters_diff <-rbind(parameters_diff,data.frame(seed=xASD$seed[x], 
                                                     diff_Estimate=xTD$Estimate[x]-xASD$Estimate[x],
                                                     diff_Est.Error=xTD$Est.Error[x]-xASD$Est.Error[x],
                                                     diff_Q2.5=xTD$Q2.5[x]-xASD$Q2.5[x],
                                                     diff_Q97.5=xTD$Q97.5[x]-xASD$Q97.5[x])) %>% 
    na.omit()
  
} 

parameters_diff %>% 
  ggplot(aes(x = seed, y = diff_Estimate, ymin = diff_Q2.5, ymax = diff_Q97.5)) +
...
parameters_diff %>% 
  mutate(check = ifelse(diff_Q2.5 > 0, 1, 0)) %>% 
