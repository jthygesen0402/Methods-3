...
pp_check(MLU_prior_model, ndraws =100) + coord_cartesian(xlim = c(-1, 60))
```
...
parameters <- mutate(estimate = map(fit, ~ fixef(.) %>% 
                           data.frame() %>%
...
sample60 %>% filter(parameters=="diagnosis")
...
```{r}
sample70 %>% filter(parameters == "diagnosis") %>% 
  ggplot(aes(x = seed, y = Estimate, ymin = Q2.5, ymax = Q97.5)) +
  geom_hline(yintercept = c(0, .5), color = "white") +
  geom_pointrange(fatten = 1/2) +
  labs(x = "seed (i.e., simulation index)",
...
```{r}
#Adding moms' MLU's as a predictor
f_real2 <- bf(MLU ~ 0 + diagnosis + diagnosis:visit + diagnosis:MOT_MLU+ (1+visit|ID))

mom_model <- update(real_model,
                    f_real2,
                    newdata = df_real_data)
```

```{r}
hypothesis(mom_model, "diagnosisTD:MOT_MLU > diagnosisASD:MOT_MLU")
```
...
```{r}
pacman::p_load(gganimate, ggplot2, tidybayes, ggdist, magrittr, tidyr, forcats, modelr, cowplot, ggrepel, posterior, dplyr, transformr)
...
anim <-  df %>%
  group_by(ID) %>%
  data_grid(MLU = seq_range(MLU, n = 101)) %>%
  add_predicted_draws(MLU_2) %>%
  ggplot(aes(x = visit, y = MLU, color = ordered(diagnosis), fill = ordered(diagnosis))) +
