...
mu_change_ASD <- 0.4 #beta
sigma_change_ASD <- 0.4 #sd_beta
...
simulation <- function(n){
  ...
  for(i in seq(nrow(d))){
    d$MLU[i] <- rlnorm(1, meanlog=d$intercept[i]+d$slope[i]*(d$visit[i]-1), sdlog=error)
  }
...
f1 <- bf(CHI_MLU~1+Diagnosis+(1+Diagnosis|Child.ID))
```
